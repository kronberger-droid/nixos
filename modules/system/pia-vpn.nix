...
in
{
  ...
  config = mkIf cfg.enable {
    # Install required packages for NetworkManager VPN
    ...
    # Create setup script that downloads PIA configs and sets up keyring
    environment.etc."pia-vpn/setup-pia.sh" = {
      text = ''
        #!/usr/bin/env bash
    ...
    environment.etc."pia-vpn/pia-connect.sh" = {
      text = ''
        #!/usr/bin/env bash
    ...
    environment.etc."pia-vpn/pia-disconnect.sh" = {
      text = ''
        #!/usr/bin/env bash
    ...
    environment.etc."pia-vpn/pia-status.sh" = {
      text = ''
        #!/usr/bin/env bash
    ...
    environment.etc."pia-vpn/pia-toggle.sh" = {
      text = ''
        #!/usr/bin/env bash
